...
socket.on("updateChat", function (username, data) {
    if (username === "INFO") {
        chatDisplay.innerHTML += `<div class='announcment'><span>${data}</span></div>`
    } else {
...
socket.on("updateRooms", function (rooms, newRoom) {
    ...
    for (var index in rooms) {
        ...
        // Set the active_item class for the newRoom
    const roomElement = document.getElementById(newRoom);
    if (roomElement) {
        roomElement.classList.add('active_item');
    }

    // Remove the active_item class from other rooms
    const roomElements = document.querySelectorAll('.room_card');
    roomElements.forEach((element) => {
        if (element.id !== newRoom) {
